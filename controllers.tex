...
Its design mimics the high-level self-aware model described earlier, as shown in \figref{poet-runtime}.
...
% (Mishra \etal propose adjusting the pole at runtime to improve behavior when combining POET with a machine linear approach that updates POET's resource specification at runtime \TODO{cite}.)
...
\figref{poet-phases-default} is a time series of job latency and system power power consumption when running \bench{x264} without POET in
...
In \figref{poet-phases-x264}, we run with POET enabled, using a performance target that is about half of the system's maximum capacity (\ie twice the minimum latency).
...
Mishra \etal combine POET with machine learning to reduce the amount of offline work needed to generate resource specifications, and update both the specification and pole value online as new application behavior is learned \TODO{cite}.
...
\TODO{Explain and formulate gain limits.}

\TODO{Demonstrate phases video -- need to update POET example to use performance instead of latency.}
